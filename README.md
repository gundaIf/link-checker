...
**Option B: static page + Worker.** Host `index.html` on GitHub Pages, deploy the API with `npx wrangler deploy worker/proxy.js --name anchorage-api`, and set `API_BASE` at the top of the script in `index.html` to the Worker's URL. Set `ALLOWED_ORIGIN` in the worker to your Pages origin.
